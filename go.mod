module github.com/ion-channel/ion-connect

go 1.15

require (
	github.com/franela/goblin v0.0.0-20201006155558-6240afcb2eb7 // indirect
	github.com/hashicorp/hcl v1.0.1-0.20190611123218-cf7d376da96d // indirect
	github.com/ion-channel/ionic v0.20.1-0.20210610163608-c8af1ccf665b
	github.com/onsi/gomega v1.10.4 // indirect
	github.com/pelletier/go-toml v1.4.1-0.20190910035959-26fd12ff5458 // indirect
	github.com/spdx/tools-golang v0.0.0-20201122192914-a16d50ee1552
	github.com/spf13/afero v1.2.2 // indirect
	github.com/spf13/cast v1.3.1-0.20190531093228-c01685bb8421 // indirect
	github.com/spf13/cobra v1.1.1
	github.com/spf13/jwalterweatherman v1.1.0 // indirect
	github.com/spf13/viper v1.7.1
	golang.org/x/crypto v0.0.0-20200622213623-75b288015ac9
	gopkg.in/yaml.v2 v2.3.0
)

replace github.com/spdx/tools-golang => github.com/ion-channel/tools-golang v0.0.0-20210315174624-e403235229f9
